<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                label="操作">
                <template slot-scope="scope">
                <el-button v-if="isAuth('xuesheng','转积极分子') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="jijifenziCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'转积极分子':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','转积极分子') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="jijifenziCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'转积极分子':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','转积极分子') && contents.tableBtnIcon == 0" type="success" size="mini" @click="jijifenziCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'转积极分子':'' }}</el-button>
    ...
    <jijifenzi-cross-add-or-update v-if="jijifenziCrossAddOrUpdateFlag" :parent="this" ref="jijifenziCrossaddOrUpdate"></jijifenzi-cross-add-or-update>
...
import jijifenziCrossAddOrUpdate from "../jijifenzi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      jijifenziCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    jijifenziCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.jijifenziCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.jijifenziCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.jijifenziCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
